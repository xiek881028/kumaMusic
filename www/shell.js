...
const assets = {
	...
	'font-awesome': {css: 'css/font-awesome.min.css', fonts: 'fonts'},
	'better-scroll': {js: 'dist'},
	axios: {js: ['dist/axios.min.js', 'dist/axios.js']},
		...
		Object.keys(assets).map((item)=>{
			...
			Object.keys(assets[item]).map((_item)=>{
				...
				if(publicFn.isArray(_asset)){
					...
					_asset.map((__item)=>{
						let isDirectory = fs.statSync(path.resolve(__dirname, rootPath, item, __item)).isDirectory();
						_fs[isDirectory? 'copyRecursive' : 'copy'](path.resolve(__dirname, rootPath, item, __item), path.resolve(assetsPath, item, _item, isDirectory? '' : path.parse(__item).base), (err)=>{
							if(err) throw err;
					...
					fs.mkdirSync(path.resolve(assetsPath, item, _item));
					let isDirectory = fs.statSync(path.resolve(__dirname, rootPath, item, _asset)).isDirectory();
					_fs[isDirectory? 'copyRecursive' : 'copy'](path.resolve(__dirname, rootPath, item, _asset), path.resolve(assetsPath, item, _item, isDirectory? '' : path.parse(_asset).base), (err)=>{
						if(err) throw err;
