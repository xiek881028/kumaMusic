...
const getUrl = async (ids, infoFlag = false) => {
  ...
  if (ids[0] != 0) {
    try {
      ...
      let vkey = await axios.request({
        url: `https://c.y.qq.com/base/fcgi-bin/fcg_music_express_mobile3.fcg`,
        params: {
          format: 'json',
          songmid: ids[2],
          filename: `C400${ids[2]}.m4a`,
          guid: 6442406400,
          platform: 'yqq',
          loginUin: 0,
          hostUin: 0,
      ...
      let songVkey = vkey.data.data.items[0].vkey;
      if (infoFlag) {
        url = {};
        url.url = `http://dl.stream.qqmusic.qq.com/C400${ids[2]}.m4a?vkey=${songVkey}&guid=6442406400&uin=0&fromtag=66`;
        url.name = data.data.data[0].title;
        url.artists = data.data.data[0].singer.map((_item) => {
          return _item.name;
        }).join('/');
      } else {
        url = `http://dl.stream.qqmusic.qq.com/C400${ids[2]}.m4a?vkey=${songVkey}&guid=6442406400&uin=0&fromtag=66`;
      }
    } catch (err) {
      url = '';
    }
